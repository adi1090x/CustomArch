...
MENU="$(rofi -sep "|" -dmenu -i -p 'Select' -location 5 -columns 1 -xoffset -28 -yoffset -80 -width 12 -hide-scrollbar -line-padding 4 -padding 20 -lines 10 <<< "> amber|> blue|> blue-grey|> brown|> cyan|> deep-orange|> deep-purple|> green|> grey|> indigo|> lime|> orange|> pink|> purple|> red|> teal|> yellow")"
            case "$MENU" in
				## Colors
